"""WR position projector.

Combines team passing volume, target share, and efficiency rates
into per-game and season-total stat projections for wide receivers.
Includes optional rushing stats for jet-sweep / gadget usage.
"""
...
def project_wr(
    ...
    """Project WR stats for the target season."""
    wrs = role_df[role_df["position"] == "WR"]
    projections: list[StatProjection] = []

    for _, wr in wrs.iterrows():
        pid = wr["canonical_player_id"]
        team = wr["team"]
        is_rookie = bool(wr["is_rookie"])
        is_tc = bool(wr["is_team_changer"])
        reasons: list[str] = []
